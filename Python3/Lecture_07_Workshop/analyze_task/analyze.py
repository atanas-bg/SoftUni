...
def main():
    ...
    load_sales_generator_object = load_sales('sales-10K.csv')
    ...
    for sale_info in load_sales_generator_object:  # see sales.load_sales() for details
        for a in analyzers:
            a.analyze_sale(sale_info)
...
class BaseAnalyzer:
    def analyze_sale(self, sale_info):
        ...
...
class TotalsAnalyzer(BaseAnalyzer):
    def __init__(self):
        super().__init__()
        self.total_sales_sum = 0
        self.total_sales_count = 0
        self.min_timestamp = None
        self.max_timestamp = None
        self.average_sales_price = 0

    def analyze_sale(self, sale_info):
        price = sale_info[KEY_PRICE]
        self.total_sales_sum += price
        self.total_sales_count += 1
        datetime_of_sale = sale_info[KEY_TIME_STAMP]
        if self.min_timestamp is None or self.min_timestamp > datetime_of_sale:
            self.min_timestamp = datetime_of_sale
        # min_timestamp = min(sale_info[KEY_TIME_STAMP] for sale_info in sales)
        if self.max_timestamp is None or self.max_timestamp < datetime_of_sale:
            self.max_timestamp = datetime_of_sale

    def print_results(self):
        self.average_sales_price = self.total_sales_sum / self.total_sales_count if self.total_sales_count else None  # тернарен оператор
        result = """
    Обобщение
    ---------
    ...
    """.format(
            sales_count=self.total_sales_count,
            total_sales=self.total_sales_sum,
            average_sales_price=self.average_sales_price,
            first_datetime_of_sale=self.min_timestamp,
            last_datetime_of_sale=self.max_timestamp)
...
class AmountsGroupedAnalyzer(BaseAnalyzer):
    ...
    def analyze_sale(self, sale_info):
        ...
        price = sale_info[KEY_PRICE]
        group_by_value = self.get_group_by_value(sale_info)
        if group_by_value in self.amounts_grouped:
            self.amounts_grouped[group_by_value] += price
        else:
            self.amounts_grouped[group_by_value] = 0
            self.amounts_grouped[group_by_value] += price

    def get_group_by_value(self, sale_info):
        ...

    def print_results(self):
        amounts_by_group_value_sorted = list(self.amounts_grouped.items())
        amounts_by_group_value_sorted.sort(key=lambda el: el[1], reverse=True)
    ...
    """.format(self.group_by_title))
        ...
        for group_value, sales_sum in amounts_by_group_value_sorted[:TOP_RESULT]:
            print('    {group_value}: {sales_sum:.2f} €'.format(group_value=group_value, sales_sum=sales_sum))
...
class AmountsByHourAnalyzer(AmountsGroupedAnalyzer):
    ...
    def get_group_by_value(self, sale_info):
        time_stamp = sale_info[KEY_TIME_STAMP]
...
def print_total_stats(sales: list):
    ...
    average_sales_price = total_sales_sum / total_sales_count if total_sales_count else None  # тернарен оператор
