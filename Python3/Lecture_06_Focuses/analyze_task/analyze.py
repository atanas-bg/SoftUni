...
def main():
    ...
    amounts_by_category_sorted = list(group_by_category.items())
    amounts_by_category_sorted.sort(key=lambda el: el[1], reverse=True)
    ...
    for category, sales_sum in amounts_by_category_sorted[:TOP_RESULT]:
...
def print_top_by_hour(sales: list):
    ...
    for sales_info in sales:
        ...
        if time_stamp not in group_sales_by_hour:
